...
description: "Shopifyテーマの新規セクション設計・プランニング。テーマプロファイル（docs/theme-profile.md）を参照し、既存CSS/JS/コンポーネントを流用した設計書を作成する。全テーマ対応。コード生成は行わない。使用タイミング：「〇〇なセクションを設計して」「セクションをプランニングして」「セクション設計書」「設計書を作って」「仕様書を書いて」「スペック」「spec 作って」「section plan」。Figmaデザインからの変換、theme blocks（@theme）の設計判断にも対応。前提：shopify-theme-analyzerでテーマ分析済みであること。"
---
...
- `docs/theme-profile.md`（または旧 `document/theme-profile.md`）が存在すること（`shopify-theme-analyzer` で生成）
...
2. テキスト入力 / Figma入力から要件を収集:
   ...
   - 繰り返し要素の有無（blocks候補。方式は Phase 3「blocks の設計判断」で決める）
   - レスポンシブ要件
...
- 重複読み込み防止パターンの選択
- "c-" プレフィックスルールの適用
- blocks の方式選択（下記「blocks の設計判断」を設計書に明記）

#### blocks の設計判断

繰り返し要素（blocks）を持つセクションでは、2方式のどちらを採るかを設計書に明記する:

| 方式 | 定義場所 | 向いているケース |
|------|---------|----------------|
| ローカル定義 blocks（従来型） | セクション schema 内に `type` + `name` で定義 | このセクション専用のブロック。テーマに `blocks/` ディレクトリが無い世代（Dawn 等） |
| `{"type": "@theme"}`（theme blocks） | テーマルート `blocks/*.liquid` を流用 | theme blocks 採用テーマ（Horizon 世代）。既存テーマブロックの流用でエディタ体験を統一できる |

選択基準:

- テーマプロファイル（docs/theme-profile.md）の「theme blocks」欄を確認。`blocks/` ディレクトリがあるテーマでは `@theme` の活用を優先検討
- `blocks/` が無いテーマではローカル定義一択
- **併用不可**: 同一セクションでローカル定義ブロックと `@theme` は併用できない（Shopify 仕様）
- `@theme` エントリは `type` のみ指定（`name` / `settings` 付与不可・重複宣言不可）。詳細は `shopify-schema-validator` の section-schema-rules を参照

### Phase 4: レビュー確認

- 設計書を提示し、修正点がないか確認
- **既存コンポーネント流用箇所**が正しいか重点確認
- OK なら `docs/c-[section-name]-spec.md` に保存

## "c-" プレフィックスルール

新規作成するセクションには "c-" を付与し、テーマ元のセクションと区別する。

> 正本: `theme-orchestrator` の「"c-" プレフィックスルール」（更新はそちらを先に。この表は写し）
