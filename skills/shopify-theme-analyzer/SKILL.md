...
description: "Shopifyテーマの構造を自動分析し、テーマプロファイル（docs/theme-profile.md）を生成する。CSS命名規則、ブレークポイント、再利用可能なコンポーネント（カルーセル、スライダー等）、変更禁止ファイル一覧を抽出。使用タイミング：新しいShopifyプロジェクト開始時、「テーマを分析して」「theme analyze」。セクション設計（shopify-section-planner）の前提として実行。"
---
...
- 変更禁止ファイルを明確化
- 結果を `docs/theme-profile.md` に保存し、`shopify-section-planner` から参照可能にする
...
| theme blocks | テーマルート `blocks/` ディレクトリの有無を確認。存在すれば **theme blocks 採用テーマ（Horizon 世代）** と判定し、`blocks/*.liquid` の一覧（名前 + 用途）を取得 |
...
theme blocks: [あり（blocks/ N ファイル） / なし]
再利用可能コンポーネント:
...
[references/theme-profile-template.md](references/theme-profile-template.md) に従い `docs/theme-profile.md` を生成・保存する。
...
**verify_mode の3モード（要約）:**

- `manual` — 自動発火しない。強制実行キーワードでのみ起動
- `smart`（デフォルト） — 大きな変更（新規 `.liquid` 作成 or diff が `smart_diff_threshold` 超え）のときだけ自動起動
- `auto` — 記録された Shopify ファイル編集が1件でもあれば必ず起動（旧挙動）

詳細な判定条件とオーバーライドキーワード（強制スキップ / 強制実行）は README の「自動検証の仕組み」を参照。
