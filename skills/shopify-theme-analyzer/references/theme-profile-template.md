...
このテンプレートに従い `docs/theme-profile.md` を生成する。
...
使用: (あり / なし — テーマルートに `blocks/` ディレクトリが存在するか)

### blocks/ 一覧

（「使用: あり」の場合のみ記載。1列目は必ずバッククォートで囲む）

| ブロック | ファイル | 用途 |
|---------|--------|------|
| `product-card` | `blocks/product-card.liquid` | (例) 商品カード |

- セクション schema で `{"type": "@theme"}` を宣言すると全テーマブロックを受け入れる
- ローカル定義ブロック（`name` あり）と `@theme` は同一セクションで併用不可
