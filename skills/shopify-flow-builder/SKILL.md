...
description: Shopify Flow（Order paid / Order created / Customer created / Scheduled time などのトリガーを起点とした、顧客メタフィールド更新・顧客タグ付与・Discount コード検知・販売開始日に応じたチャネル自動公開・在庫切れ自動アーカイブなどのワークフロー）をゼロから構築するためのベストプラクティス・パターン集・構造テンプレート集。Use this skill whenever a user mentions Shopify Flow 作成, ワークフロー自動化, Order paid トリガー, Order created トリガー, Scheduled time トリガー, スケジュールトリガー, 定期実行 Flow, cron Flow, customer metafield 自動更新, 顧客タグ自動付与, 商品タグ自動付与, クーポン配布自動化, Thanks Coupon, ロイヤルティポイント付与, Discount code 使用検知, 販売開始日 自動公開, リリース日 チャネル公開, Meta 自動公開, Google 自動公開, Facebook & Instagram 自動公開, Google & YouTube 自動公開, Publish product action, Unpublish product action, Get product list action, Get customer list action, Publication ID 取得, Run Code action, Shopify Flow テンプレート, .flow ファイル, Flow エクスポート, あるいは具体的に「注文時に〇〇を更新する Flow」「特定タグの注文に〇〇する Flow」「金額帯に応じて顧客 MF に値を書き込む Flow」「クーポン使用時に〇〇をクリアする Flow」「リピート購入で自動的にタグを付ける Flow」「リリース日を過ぎたら自動でチャネル公開する Flow」「販売終了日で自動非公開する Flow」「在庫切れ商品を自動アーカイブする Flow」「N 日経過した商品の new タグを自動削除する Flow」などの依頼が出た時。Shopify Flow を Visual Builder でゼロから構築するあらゆる場面で発火させる。.flow JSON のインポート・エクスポート・バージョン管理の話題でも必ず発火させる。
---
