...
| スキル | トリガー | 入力 | 出力 |
|--------|---------|------|------|
| **shopify-store-bootstrap** | `新しいストア案件を立ち上げて` / `新案件のストア準備` / `store bootstrap` | テーマファイル一式（git 未初期化） | git init〜first commit・toml store固定・テーマ分析・memory登録・GitHub private repo までのフルセットアップ |
...
2つの Hook と 1つの Agent が連携して、テーマファイル編集後の検証を自動化する:
...
残る1つの Hook **shopify-theme-context.sh** (SessionStart) は検証パイプラインとは独立で、セッション開始時にテーマプロファイル等のコンテキストを注入する。
...
`shopify_paths` を省略した場合、フックは標準テーマディレクトリ（`sections/` `snippets/` `blocks/` `templates/` `layout/` `assets/` 等）にフォールバックする。
...
| `shopify_paths` | Shopifyテーマファイルとみなす glob パターン。編集記録 Hook が検証対象の判定に使う。省略時は標準テーマディレクトリにフォールバック |
...
monolith (❌)                  modular (✅)
┌──────────────────┐          ┌──────┐ ┌──────────┐ ┌─────────┐
│ init              │          │ init │ │ analyzer │ │ planner │
│ analyze           │          └──────┘ └──────────┘ └─────────┘
│ plan              │    →     ┌──────────────┐ ┌───────────┐
│ implement         │          │ orchestrator │ │ validator │
│ validate          │          └──────────────┘ └───────────┘
└──────────────────┘
```

現行は 14 スキル構成。コアパイプラインの 5 スキル（init: 構造整備 → analyzer: 分析 → planner: 設計 → orchestrator: 実装 → schema-validator: 検証）に、横断・別軸の 9 スキル — store-bootstrap（新案件立ち上げのフルセットアップ。init と analyzer を内包して呼び出す上位オーケストレーター）、ds-component-search（既存資産の洗い出し。orchestrator Phase 0 から自動呼び出し）、asset-harvest（実装資産の案件横断ライブラリへの回収。ds-component-search と対）、theme-brand-layer（Brand 層の設計・実装）、flow-builder（Shopify Flow 自動化）、cv-tracking（CV 計測タグ / カスタムピクセルの実装と検証）、delivery-report（クライアント向け報告文の生成と実績記録）、cli-auth（Shopify CLI のアカウント切替・認証・ストア固定）、blog-publish（Drive 原稿からブログ記事のワンショット公開）— を加えた形。
...
```
skills/
├── shopify-store-bootstrap/    新案件立ち上げオーケストレーター（init + analyzer + git/toml/memory/GitHub）
│   └── SKILL.md
├── shopify-theme-init/         プロジェクト初期化・クリーンアップ
│   ├── SKILL.md
│   └── references/
│       ├── gitignore-template.md
│       ├── shopifyignore-template.md
│       └── readme-template.md
...
│   └── SKILL.md
├── shopify-asset-harvest/      実装資産の回収 → 案件横断ライブラリ登録
│   ├── SKILL.md
│   └── references/
│       └── generalization-checklist.md  機密スクラブ・汎用化チェックリスト
...
└── shopify-blog-publish/       Drive 原稿 → Shopify ブログ記事のワンショット公開
    ├── SKILL.md
    └── references/
        ├── setup-guide.md               実行者の初回セットアップガイド
        ├── graphql-recipes.md           検証済み GraphQL operation 集
        └── kv-generation.md             KV サムネイルのテンプレート自動生成手順
